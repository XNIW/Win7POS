    ...
    public sealed class HeldCartRepository
    {
        ...
        public async Task<IReadOnlyList<HeldCartSummary>> ListHoldsAsync()
        {
            ...
            var rows = await conn.QueryAsync<HeldCartSummary>(
                "SELECT holdId AS HoldId, createdAtMs AS CreatedAtMs, totalMinor AS TotalMinor FROM held_carts ORDER BY createdAtMs DESC").ConfigureAwait(false);
            return rows.ToList();
        }
        ...
        public async Task<IReadOnlyList<HeldCartLineRow>> LoadHoldLinesAsync(string holdId)
        {
            ...
            var rows = await conn.QueryAsync<HeldCartLineRow>(
                ...
                new { holdId }).ConfigureAwait(false);
            return rows.ToList();
        }
        ...
        {
            using var conn = _factory.Open();
            await conn.ExecuteAsync("DELETE FROM held_carts WHERE holdId = @holdId", new { holdId }).ConfigureAwait(false);
        }
    }
