...
if ($sync -notmatch "GetPersistedReversalEconomicsErrorAsync" -or $sync -notmatch "MarkPreflightBlockedAsync\(item, economicsError") { Fail "legacy immutable payload preflight missing" } else { Pass "legacy gross-only payloads are blocked before prepare/network" }
