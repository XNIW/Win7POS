...
Assert-Contains $reconciliation "CatalogImportInProgressLeaseMilliseconds" "catalog import reconciliation reuses outbox lease"
Assert-Contains $reconciliation "RecoverExpiredInProgressAsync" "catalog import reconciliation recovery present"
...
Assert-Contains $repository "expectedAttemptCount <= 0" "catalog import final transitions require explicit attempt token"
...
Assert-Contains $initializer "server_request_id" "catalog import server request id column present"
Assert-Contains $initializer "catalog_import_client_item_id" "catalog import price history client item id column present"
...
Assert-Contains $syncService "batch.IdempotencyKey" "catalog import sync requires remote idempotency key before ack"
Assert-Contains $syncService "CatalogImportAckResult" "catalog import sync builds ack result"
Assert-Contains $syncService "MarkAckedAsync" "catalog import sync can ack remote acceptance"
Assert-Contains $repository "ApplyRemoteProductIdsAsync" "catalog import ack applies remote product ids"
