...
$limits = Read-RepoFile "src/Win7POS.Data/Import/SupplierExcelImportLimits.cs"
...
Assert-Contains $viewModel "CancelCommand = new RelayCommand(Cancel, () => !IsBusy || _analyzeCancellation != null)" "Cancel must remain available only during supplier analysis or while idle."
Assert-Contains $viewModel "cancellation.Cancel();" "Busy Cancel must request cooperative supplier analysis cancellation."
Assert-Contains $viewModel "statusAnalysisCancelled" "Cancelled supplier analysis must leave the wizard reusable."
Assert-Contains $viewModel "LocalizedReaderError" "Supplier reader error codes must be localized at the WPF boundary."
Assert-Contains $workflow "ReadFirstWorksheet(filePath, cancellationToken)" "Supplier workflow must pass cancellation into the reader."
Assert-Contains $workflow "SupplierImportAnalyzer.Analyze(table, products, cancellationToken" "Supplier workflow must pass cancellation into final analysis."
Assert-Contains $analyzer "MaxHeaderLookbackRows = 2" "Supplier header lookback must remain bounded to two rows."
Assert-Contains $analyzer "NormalizedHeaderFragments" "Supplier analyzer must support exact compound-header fragments."
Assert-Contains $analyzer "HeaderMatchesAlias" "Supplier analyzer must keep alias matching exact and centralized."
Assert-Contains $analyzer "AmbiguityMargin = 0.08" "Supplier candidate scoring must retain an explicit ambiguity margin."
Assert-Contains $analyzer "ShouldAssignCandidate" "Supplier ambiguous columns must not be assigned without the score margin."
Assert-Contains $analyzer "candidate.Reasons.Take(4)" "Supplier candidate reason traces must remain bounded."
Assert-Contains $workflow "LogDetectionTrace(analysis.DetectionTrace)" "Supplier workflow must emit bounded structural detection diagnostics."
Assert-Contains $workflow "decision.Candidates.Take(3)" "Supplier diagnostic logging must cap candidates per field."
Assert-NotContains $reader "AsDataSet" "Supplier reader must not materialize DataSet objects."
Assert-Contains $reader "while (reader.NextResult())" "Worksheet count must use result traversal without cell matrices."
Assert-Contains $reader "BuildRawTableFromOwnedRows" "Reader rows must transfer ownership into raw-table construction."
Assert-Contains $limits "MaximumInputFileBytes = 32L * 1024L * 1024L" "Supplier file-size limit must remain centralized."
Assert-Contains $limits "MaximumWorksheetRows = 60000" "Supplier row limit must remain centralized."
Assert-Contains $limits "MaximumWorksheetCells = 1000000L" "Supplier cell limit must remain centralized."
Assert-Contains $limits "MaximumOoxmlMetadataXmlBytes = 4L * 1024L * 1024L" "Supplier OOXML metadata expansion limit must remain centralized."
Assert-Contains $limits "MaximumHtmlTableCandidates = 4096" "Supplier HTML table preselection must remain bounded."
Assert-Contains $localization "supplierExcelImport.errorCorruptOrUnsupported" "Recoverable supplier reader errors need localized operator text."
Assert-Contains $workflow "BuildSyncPreviewAsync" "Workflow must build DB sync preview."
Assert-Contains $workflow ".BuildPreviewAsync(rows)" "Workflow Step 4 must capture products and shop context atomically."
Assert-Contains $workflow "rebuilt.Fingerprint" "Workflow must recompute and verify Step 4 before writing."
Assert-Contains $applier "SupplierImportAnalyzer.BuildSyncPreview" "Data applier must rebuild Step 4 inside the transaction."
Assert-Contains $applier "TryMatchApplyExpectations" "Data applier must compare the transactional snapshot with the Step 4 baseline."
Assert-Contains $applier "CatalogShopTransitionBarrier" "Supplier Step 4 and Apply must serialize with catalog shop transitions."
Assert-Contains $applier "LoadApplyContextAsync" "Supplier Apply must revalidate official shop context and transition epoch."
Assert-Contains $applier "CatalogShopStateRepository.TransitionEpochKey" "Supplier Apply context must include the catalog transition epoch."
Assert-Contains $applier "ParameterBatchSize = 500" "Data applier must keep barcode batching centralized and bounded."
Assert-Contains $applier "command.Prepare()" "Data applier must prepare repeated commands."
...
Assert-Contains $wpfSmoke "VerifyAnalyzeCancellationAsync" "WPF supplier smoke must cancel Analyze and reuse the wizard."
Assert-Contains $wpfSmoke "cancellationPass" "WPF supplier smoke report must include cancellation proof."
