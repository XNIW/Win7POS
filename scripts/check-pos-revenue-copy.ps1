...
$paymentCopy = $paymentXaml + $paymentVm + $localization + $secondaryLocalization
...
if ($paymentCopy -notmatch "Boleta/PDF locale") { Fail "payment view must label local boleta/PDF" } else { Pass "local boleta/PDF label present" }
...
if ($paymentCopy -notmatch "Stampa automatica boleta solo con contanti") { Fail "auto-print cash policy copy missing" } else { Pass "auto-print cash policy copy present" }
...
if ($registerCopy -notmatch "Documento: Boleta PDF stampata" -or $registerCopy -notmatch "Non stampata \(contanti\)" -or $registerCopy -notmatch "Non stampata \(policy carta\)") { Fail "register detail document status copy missing" } else { Pass "register document status copy present" }
