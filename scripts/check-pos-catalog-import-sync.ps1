...
Require "sync service validates mandatory remote payload hash before ack" $service "payload_hash_mismatch"
Require "sync service validates mandatory remote attempt count before ack" $service "if\s*\(batch\.AttemptCount\s*!=\s*item\.AttemptCount\s*\+\s*1\)"
Require "sync service validates authoritative response shop before ack" $service "GetResponseShopMismatchCode"
