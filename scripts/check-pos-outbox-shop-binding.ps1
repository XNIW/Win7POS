...
if (-not $enqueueFacadeDelegates -or
    $salesOutboxRepository -notmatch "OutboxShopBinding\.ResolveRequiredAsync\(conn, tx\)" -or
    $catalogRepository -notmatch "ResolveRequiredAsync") { Fail "one or more enqueue paths do not capture official binding" } else { Pass "sales and catalog enqueue capture official binding" }
...
if (-not $prepareFacadeDelegates -or
    $salesOutboxRepository -notmatch "status = 'in_progress'" -or
    $salesOutboxRepository -notmatch "attempt_count = @expectedAttemptCount" -or
    $salesOutboxRepository -notmatch "MarkAckedAsync" -or
    $salesSync -notmatch "client_batch_mismatch" -or $salesSync -notmatch "response_shop_mismatch") { Fail "sales attempt/ACK guards incomplete" } else { Pass "sales prepare and ACK are attempt/shop guarded through F4" }
if ($catalogRepository -notmatch "CatalogImportInProgressLeaseMilliseconds" -or
