...
if ($catalogSync -notmatch "GetMismatchCode[\s\S]*MarkOriginBlockedAsync[\s\S]*PrepareAttemptAsync") { Fail "catalog drain does not block mismatch before prepare" } else { Pass "catalog drain blocks mismatch before prepare/send" }
if ($salesSync -notmatch "GetMismatchCode[\s\S]*MarkPreflightBlockedAsync[\s\S]*PrepareSalesSyncAttemptAsync[\s\S]*SalesSyncAsync") { Fail "sales drain does not block mismatch before prepare/send" } else { Pass "sales drain blocks mismatch before prepare/send" }
...
if ($saleRepository -notmatch "payload_json = @payloadJson" -or $saleRepository -notmatch "payload_hash = @payloadHash" -or $salesSync -notmatch "Sha256Hex\(item\.PayloadJson\)" -or $salesSync -notmatch "payload_hash_mismatch") { Fail "sales immutable payload/hash guards incomplete" } else { Pass "sales payload/hash are captured at enqueue and verified on retry" }
if ($saleRepository -notmatch "IsReversalDependencyReadyAsync" -or $saleRepository -notmatch "ValidateReversalBoundaryAsync" -or $saleRepository -notmatch "OriginalOriginShopCode" -or $salesSync -notmatch "original_sale_not_acked") { Fail "reversal boundary/dependency shop guards incomplete" } else { Pass "reversal boundary requires original ACK and matching official/current origin" }
