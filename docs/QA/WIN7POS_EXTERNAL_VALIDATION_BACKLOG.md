...
- Backlog status: `OPEN`.
- External validation status: `EPSON_TRANSACTIONAL_PHYSICAL_PASS_ADDENDUM_OPEN`.
- Software merge authorization: `APPROVED_BY_PROJECT_OWNER`.
- Production/hardware certification: `OPEN` and `NOT_YET_CERTIFIED`.

Rows 15 and 17–22 are the only rows declared physically PASS. Every other open
...
| 19 | Epson transactional cash receipt and cutter; drawer opens once. | `PASS_PHYSICAL_SINGLE_OPEN_2026-07-18` | `YES` | `BLOCKED_BY_RECEIPT_SURFACE_ADDENDUM` | `PASS_FOR_TESTED_QA_SCENARIO` |
| 20 | Epson card-only receipt; drawer remains closed. | `PASS_PHYSICAL_NO_DRAWER_2026-07-18` | `YES` | `BLOCKED_BY_RECEIPT_SURFACE_ADDENDUM` | `PASS_FOR_TESTED_QA_SCENARIO` |
| 21 | Persisted-sale receipt reprint; no new sale and drawer remains closed. | `PASS_PHYSICAL_NO_DRAWER_2026-07-18` | `YES` | `BLOCKED_BY_RECEIPT_SURFACE_ADDENDUM` | `PASS_FOR_TESTED_QA_SCENARIO` |
| 22 | Paused queue commits before print failure; resumed reprint has no duplicate and no drawer opening. | `PASS_COMMIT_BEFORE_PRINT_NO_DUPLICATE_2026-07-18` | `YES` | `BLOCKED_BY_RECEIPT_SURFACE_ADDENDUM` | `PASS_FOR_TESTED_QA_SCENARIO` |
...
transactional drawer behavior or physical Windows 7 validation. Rows 19–22 now close the tested QA cash,
card, reprint and paused/resumed-queue matrix with operator-confirmed paper and
drawer behavior. They do not close mixed payment, disconnected drawer, the new
receipt-surface/daily-close addendum or physical Windows 7 validation.
